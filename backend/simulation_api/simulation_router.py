...
from core.lns2_simulation_core import LNS2SimulationCore
from core.parking_manager import ParkingManager
from core.lacam0_parking_manager import LaCAM0ParkingManager
from core.lns2_parking_manager import LNS2ParkingManager
...
from planning.lns2_batch_planner import LNS2BatchPlanner
...
def start_simulation(req: LiveSimulationRequest, db: Session = Depends(get_db)):
    ...
    def simulation_factory():
        ...
        if req.algorithm == "lns2":
            ghost_mgr = GhostExitManager(grid.width, grid.height, exit_cells)
            pm = LNS2ParkingManager(
                grid=grid,
                parking_cells=parking_cells,
                exit_cells=exit_cells,
                entry_cells=entry_cells,
            ...
            batch_planner = LNS2BatchPlanner()
            return LNS2SimulationCore(
                grid=grid,
                parking_manager=pm,
                config=cfg,
                batch_planner=batch_planner,
...
def _build_and_run_headless(req: HeadlessSimulationRequest, grid: Grid) -> HeadlessResultDTO:
    ...
    if req.algorithm == "lacam0":
        ...
        simulation = LNS2SimulationCore(
            grid=grid, parking_manager=pm, config=cfg,
            batch_planner=LNS2BatchPlanner(), ghost_exit_manager=ghost_mgr,
        )
    else:
        pm = ParkingManager(
            grid=grid, parking_cells=parking_cells,
