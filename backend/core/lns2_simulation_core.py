...
class LNS2SimulationCore:
    """
    Lifelong simulation orchestrator designed for LNS2.
    ...
    - No per-car conflict resolution layer — LNS2 guarantees conflict-free paths.
    - Planning is done in batches: one LNS2 call for ALL active cars at once.
    ...
    def _trigger_replan_if_needed(self):
        """Call LNS2 in batch for all active cars when a replan is warranted."""
        ...
        if paths is not None:
            ...
            # LNS2 failed: cars keep existing paths (or wait with no path).
            self.total_failed_plans += 1
            self.needs_replan = False
            self.last_replan_time = self.time
    ...
    def _execute_paths(self):
        """Advance every active car by one step along its LNS2 path."""
